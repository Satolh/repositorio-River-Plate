...
import gameDetails from "../TorneoLocal/TorneoDetailsGames.json"
...
const DatoPartidosTorneo = () => {
  ...
  useEffect(()=>{
    ...
    const dateSelected =  localStorage.getItem("gameSelectedTorneo")
    const nameTorneo = localStorage.getItem("torneo")
    setNameTorneoLocal(nameTorneo)
    console.log(dateSelected)
    const partidoSelected = gameDetails.filter(item => item.date == dateSelected);
    setGameSelected(partidoSelected)
    console.log(partidoSelected)
  },[])
  ...
  useEffect(()=>{
    if(gameSelected[0]){
      ...
      const suplentesNumberVisitante = gameSelected[0].visitante.lineUpNumber.slice(11)
        ...
        console.log(detailsGameSelected)
  },[detailsGameSelected])
  useEffect(() => {
    ...
    <section className='container-dato-partido'>
      ...
      { gameSelected[0] &&
        <div className='container-all-games'>
            <h1 className='title-partido'>{`Torneo Local ${nameTorneoLocal}`}</h1>
          <section className='container-teams'>
            ...
            <div className='container-info-vs'>
              ...
              <p className='vs-result'>{gameSelected[0].title}</p>
            </div>
...
export default DatoPartidosTorneo;